...
char *list_users(const User *curr) {
    char *result = NULL;
    size_t len = 0;
    FILE *memstream = open_memstream(&result, &len);
    
    if (memstream == NULL) {
        return NULL;
    }
    
    fprintf(memstream, "User List\n");
    while (curr != NULL) {
        fprintf(memstream, "\t%s\n", curr->name);
        curr = curr->next;
    }
...
char *print_post(const Post *post) {
    if (post == NULL) {
        ...
        strcpy(error, "Error: Post is NULL");
        return error;
    }
    
    size_t total_len = strlen(post->author) + strlen(post->contents) + 100; // 100 for date and formatting
...
char *print_user(const User *user) {
    if (user == NULL) {
        return strdup("Error: User is NULL");
    }
    ...
    fprintf(memstream, "------------------------------------------\n");
    fprintf(memstream, "Posts:\n");
    
    const Post *curr_post = user->first_post;
    while (curr_post != NULL) {
        fprintf(memstream, "From: %s\nDate: %s\n%s\n", 
                curr_post->author, 
                asctime(localtime(curr_post->date)),
                curr_post->contents);
